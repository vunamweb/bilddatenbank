...
function fpd($feld, $val, $art, $pflicht, $breite=0) {
	global $form_desc;

	$pd 	= explode("\n", $val);

	if ($art == "sel") {
		$tmp = '<select name="'.$feld.'" class="form-control" '.$pflicht.'><option value="">bitte w&auml;hlen</option>';
		$form_desc = $fnm;
	}
	...
	foreach($pd as $val) {
		...
		if ($art == "sel") {
			if ($val == $post) $sel = "selected";
			else $sel = "";
			$tmp .= "<option value=\"$val\" $sel>$anz</option>\n";
		}

		elseif ($art == "radio" && $val) {
			$tmp .= '			<span class="radioform"><input type="radio" id="'.str_replace(" ", "_", $val).'" value="'.$val.'" name="'.$feld.'"'. ($x < 1 && $pflicht ? ' validate="required:true"' : '') .' /> &nbsp; '.$anz.'</span>
';
			$x++;
		}
	}
...
*/
	...
	$dsText = '';
	if($lan == "de") $dsText = '<span class="formspan small">Ich stimme zu, dass meine Angaben aus dem Kontaktformular zur Beantwortung meiner Anfrage erhoben und verarbeitet werden. Die Daten werden nach abgeschlossener Bearbeitung Ihrer Anfrage gelöscht.<br>Hinweis: Sie können Ihre Einwilligung jederzeit für die Zukunft per E-Mail an '.$morpheus["email"].' widerrufen.<br/>
							Detaillierte Informationen zum Umgang mit Nutzerdaten finden Sie in unserer <a href="'.$dir.$navID[36].'" target="_blank"><u>Datenschutzerklärung</u></a></span>';
			...
			<div class="container">
				<div class="row">
					<div class="col-md-offset-1 col-md-10">
						...
						<form class="frmContact leftalgn" role="form" id="kontaktformular" method="post" action="'.$dirX.$lan.'/'.$navID[$cid].'" name="'.$_GET["sn2"].'">
							<input type="Hidden" name="fid" value="'.$fid.'">
							'.$form .'<br style="clear:left;" />

							<div class="col-md-12">
								<div class="mt2 mb2 form-group">'.$dsButton.$dsText.'</div></div>
							</div>
