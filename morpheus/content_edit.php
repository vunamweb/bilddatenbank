...
function pulldownContent ($tp, $db, $wname, $wid, $mf=0) {
	...
	while ($row = mysqli_fetch_object($result)) {
		...
		elseif ($db == "morp_cms_product")	$nm = $row->prokbezde ." - $nm";
		elseif ($db == "morp_cms_nav")	{
...
elseif ($save) {
	...
	foreach($_POST as $key=>$val) {
		...
		if ($key[0] == "brick") {
			if ((preg_match("/link_/", $key[1]) || preg_match("/bild/", $key[1]) || preg_match("/image_popup/", $key[1]) || preg_match("/anker_link/", $key[1]) || preg_match("/gleicher/", $key[1])) && !preg_match("/linksbuendig/", $key[1]) && !preg_match("/download/", $key[1]) && !preg_match("/TOP/", $key[1]) && !$linktext)
				$linktext = $val ."|";
		...
		if ($val) {
			...
			if ($counter == $stelle && $del)
				$textbox .= "<p style=\"background-color:#ff0000; height: 38px; line-height: 37px; margin: 0 0px 30px 0px;\"><font> &nbsp; &nbsp; <a href=\"$zuruck\">< abbrechen</a>  &nbsp; ".$MORPTEXT["CONE-LOSCHEN"]."</font>&nbsp; &nbsp;
					<input type=\"hidden\" name=\"stelle\" value=\"$counter\" class=\"form-control\">YYY<input type=\"submit\"  class=\"btn btn-default sve\" name=\"erstellen\" value=\"speichern $counter\"></p>";
...
//				elseif (preg_match("/termin/", $b_chk)) 								$insert_termin = 1;
				...
				elseif (preg_match("/html/", $b_chk) || preg_match("/termine tab/", $b_chk)) 								$row = 10;
...
# content_edit.php?db=$db&edit=$edit&stelle=$counter&split=$counter&save=1
							...
							<td> &nbsp; $counter. <a href=\"javascript:check('brick_change.php?cid=$edit&navid=$navid&back=$back&pos=$counter&db=$db');\" class=\"textedit\">" .str_replace("_", " ", $b_chk) ."</a>".' &nbsp; &nbsp; <i class="fa fa-file-text selall" ref="s'.$counter.'"></i> <span class="rahmen"><i class="fa fa-anchor structureOn" ref="s'.$counter.'"></i> <i class="fa fa-external-link externalLink" ref="s'.$counter.'"></i> <i class="fa fa-at mailLink" ref="s'.$counter.'"></i> <i class="fa fa-eraser delAllLinks" ref="s'.$counter.'"></i></span>
								...
								<table class="tbl0">'."
									<tr>
										...
										if (preg_match("/text/", $brick[0])) $textbox .= "<input type=\"submit\" name=\"split\" class=\"split mobileOff\" value=\"Text Splitten <s>\"></td>";
					...
					$textbox .= "
							<td>
								<textarea id=\"s".$counter."\" name=\"brick#" .$brick[0] ."#" .$counter ."\" style=\"height:" .($row*30) ."px;\" class=\"form-control ta getVal\" onchange=\"setchange('1');\">".stripslashes($txt)."</textarea>
							</td>
						</tr>
					</table>
					";
