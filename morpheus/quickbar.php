...
foreach ($morpheus["lan_arr"] as $key=>$lang) {
	...
	for ($i = 1; $i <= 5; $i++) {
		$sql  = "SELECT * FROM `morp_cms_nav` WHERE ebene=$i AND lang=$key";
		$res = safe_query($sql);
...
*/			}
	...
	foreach($arrSmap as $id=>$path) {
		if ($home == $id && $lang == "de") $smap .= $site_start.$url.$site_end;
		else $smap .= $site_start.$url.($multilang ? $lang.'/' : '').$path.$site_end;
	}
