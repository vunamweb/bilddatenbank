...
elseif ($save && $name) {
	...
	$set = "set name='$name', title='$title', keyw='$keyw', lnk='$link', `desc`='$desc', `fbimage`='$fbimage', design='$design', `oldlnk`='$oldlnk', `setlink`='$setlink', `shortlink`='$shortlink', datum='$datum', author='$author', blog='$blog', sichtbar=$sichtbar, bereich=$bereich, button='$button', lang=$sprache, edit=1, `lock`=".($lock ? 1 : 0).", `nocontent`=".($nocontent ? 1 : 0).", accesskey='$accesskey'";
	...
	if ($edit) {
		...
		$query 	= "INSERT `morp_cms_nav` " .$set .", ebene=$ebene, parent=$parent, sort=$sort";
		$result = safe_query($query);
