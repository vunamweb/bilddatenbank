...
function timetableWeek(el, tiva_timetables, firstDayWeek) {
	...
	// Get min, max time
	var min_time = getMinTime(tiva_timetables) < 8 ? getMinTime(tiva_timetables) : 8;
	var max_time = getMaxTime(tiva_timetables) > 18 ? getMaxTime(tiva_timetables) : 18;
						...
						for (var t = 0; t < timetables.length; t++) {
							if (timetables[t].start_time && timetables[t].end_time) {
								...
								if (timetables[t].image) {
									var timetable_image = '<div class="timetable-image"><img src="/intranet/admin/timetable/images/' + timetables[t].image + '" alt="' + timetables[t].name + '" /></div>';
								} else {
									var timetable_image = '';
								}
								...
								var item_invite = timetables[t].invite == 1 ? '<i class="fa fa-star"></i> &nbsp; ' : '';
								...
								timetableString += 	'<div class="timetable-item">'
														+ '<a class="timetable-title color-' + timetables[t].color + '" style="top:' + position + 'px; height:' + height + 'px; ' + item_width + item_left + '" href="#timetable-popup-' + timetables[t].id + '" class="open-popup-link">'
															+ '<div class="timetable-title-wrap">'
																+ '<div class="timetable-name">' + item_invite + timetables[t].name + '</div>'
														...
														+ '<div id="timetable-popup-' + timetables[t].id + '" class="timetable-popup zoom-anim-dialog mfp-hide">'
															...
															+ '<div class="popup-body">'
																...
																+ (item_invite ? '<div class="timetable-desc"><a href="/intranet/?hn=termine&sn2=gaesteliste&cont=gaesteliste&event=' + timetables[t].event + '" class="btn btn-info">' + timetables[t].inviteText + '</a></div>' : '')
															+ '</div>'
														+ '</div>'
													+ '</div>';
							}
							...
							if (timetables[t].start_time && timetables[t].end_time) {
								...
								timetableString += 	'<div class="timetable-item">'
														...
														+ '<a class="timetable-title" href="#timetable-popup-' + timetables[t].id + '" class="open-popup-link">'
															...
															+ '<span class="timetable-name">' + item_invite + timetables[t].name + '</span>'
														+ '</a>'
														+ '<div id="timetable-popup-' + timetables[t].id + '" class="timetable-popup zoom-anim-dialog mfp-hide">'
															+ '<div class="popup-header color-' + timetables[t].color + '">'
																+ timetables[t].name
															...
															+ '<div class="popup-body">'
																...
																+ '<div class="timetable-time color-' + timetables[t].color + '">' + timetables[t].start_time + timetable_end_time + '</div>'
																+ '<div class="timetable-desc">' + timetables[t].description + '</div>'
																+ (item_invite ? '<div class="timetable-desc"><a href="/intranet/index.php?hn=termine&sn2=gaesteliste&cont=gaesteliste&event=' + timetables[t].event + '" class="btn btn-info">' + timetables[t].inviteText + '</a></div>' : '')
															+ '</div>'
														+ '</div>'
													+ '</div>';
							}
...
jQuery(document).ready(function(){
	jQuery('.tiva-timetable').each(function(index) {
		...
		jQuery.ajax({
			url: '/intranet/timetables.php',
			dataType: 'json',
			data: '{mode:mode}',
			beforeSend : function(){
				timetable_contain.html('<div class="loading"><img src="/intranet/assets/images/loading.gif" /></div>');
			},
