...
$sorting_col = "name";
    ...
    while ($row = mysqli_fetch_object($res)) {
        ...
        $sql = "SELECT * FROM $table where category_id =".$tags_category_id."  order by $sorting_col_1";
        $res_1 = safe_query($sql);
        $row_1 = mysqli_fetch_object($res_1);
        
        while ($row_1 = mysqli_fetch_object($res_1)) {
           $select .= '<option value="'.$row_1->$primary_1.'">'.$row_1->$show_col_1.'</option>'; 
        }
        
        $count++;
        
        $select .= ($count < $num_rows) ? '</select><select name="select" class="ui selection dropdown" multiple="">' : '</select>';
    }

/*$select = '
<div id="sel-cont" class="sel-cont">
  ...
  <select name="select" class="ui selection dropdown" multiple="" id="multi-select-1" >
  <option value="">Wählen Sie aus...</option>
  <option value="01">Vertiefung IWM</option>
  <option value="02">WM Verwaltung</option>
  <option value="03">Datenintergration</option>
...
$path 	=  'D:/xampp-7-4/htdocs/bilddatenbank'.'/Galerie/'.$morpheus["GaleryPath"].'/'.$ordner.'/';
